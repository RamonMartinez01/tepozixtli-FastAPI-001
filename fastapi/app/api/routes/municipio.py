...
# GET /api/v1/municipios/
# ==============
@router.get("/entidad/{cve_ent}", response_model=List[MunicipioResponse])
async def get_by_entidad(
    cve_ent: str, 
    db: AsyncSession = Depends(get_db)):
    return await municipio_service.get_by_entidad(db, cve_ent=cve_ent)
...
# GET /api/v1/municipios/
# ==============
@router.get("/{cvegeo}", response_model=MunicipioResponse)
